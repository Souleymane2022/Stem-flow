...
export function ArticleSheet({ title, category, text, imageUrl, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/80 md:items-center" onClick={onClose}>
      <article
      ...
      >
        <div className={`relative bg-gradient-to-br ${meta.gradient} px-6 py-8`}>
          ...
          <span className={`rounded-full border px-3 py-1 text-[11px] font-bold ${meta.bg} ${meta.border} ${meta.text}`}>
            {meta.emoji} {category}
          </span>
          <h1 className="mt-3 text-3xl">{title}</h1>
        </div>
        {imageUrl && <img src={imageUrl} alt={title} loading="lazy" className="max-h-72 w-full object-cover" />}
